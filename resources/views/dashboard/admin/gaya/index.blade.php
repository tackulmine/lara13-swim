          ...
          <table class="table table-striped table-bordered" id="dataTableCustom" width="100%" cellspacing="0"
            ...
            <tbody>
              @foreach ($gayas as $gaya)
                @php
                  $userProfile = optional($gaya->profile);
                  $userStaffType = optional(optional($gaya->userStaff)->type);
                @endphp
                <tr>
                  <td>{{ $gaya->id }}</td>
                  <td>{{ $loop->iteration }}</td>
                  <td data-order="{{ intval($gaya->name) }}">{{ $gaya->name }}</td>
                  <td class="text-right">{{ $gaya->user_member_gaya_limits_count }}</td>
                  <td class="text-right">{{ $gaya->user_member_limits_count }}</td>
