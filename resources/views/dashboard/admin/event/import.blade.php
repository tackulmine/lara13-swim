...
@section('content')
  <div class="card shadow mb-4">
    ...
    <!-- form start -->
    {{-- {!! Form::open([
        'route' => [$baseRouteName . 'import', $event->id],
        'class' => 'form-horizontal form-disabled-submit',
        'files' => true,
    ]) !!} --}}
    {{ html()->form('POST')->route($baseRouteName . 'import', $event->id)->attributes([
            'class' => 'form-horizontal form-disabled-submit',
...
@push('js')
  ...
  <script>
    $(document).ready(function() {
      ...
      $('#delete_old_data1').on('change', function() {
        if (this.checked) {
          var returnVal = confirm(
            "YAKIN MENGHAPUS DATA LAMA? \n\nHATI-HATI!! Data tidak dapat dipulihkan kembali!!\n\nKlik 'Cancel' untuk membatalkan."
          );
          $(this).prop("checked", returnVal);
        }
      });
      $('#multiple_sheets').on('change', function() {
        var target = $('#total_sheets');
