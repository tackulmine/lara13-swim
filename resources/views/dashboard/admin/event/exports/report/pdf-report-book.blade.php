...
<body>
  ...
  @foreach ($eventStages as $eventStage)
    @php
      $participants = $eventStage->eventSessionParticipants;
    @endphp
    {{-- <div class="row">
            <div class="col-6">
                <strong>ACARA {{ $eventStage->number_format }} {{ strtoupper($eventStage->masterMatchType->name) }}</strong>
            </div>
            <div class="col-6 text-right">
                <strong>KATEGORI {{ $eventStage->masterMatchCategory->name }}</strong>
            </div>
        </div> --}}
    <table class="table table-sm table-borderless mb-1">
      ...
      <tr>
        ...
        <th width="37%">{{ __('Nama Lengkap Atlet') }}</th>
        <th width="5%" class="text-center">{{ __('Lahir') }}</th>
        <th width="30%">{{ __('Sekolah') }}</th>
        <th width="10%" class="text-center">{{ __('Prestasi') }}</th>
        {{-- <th>ACARA</th>
                    <th>TIPE</th>
                    <th>KAT.</th> --}}
        {{-- <th class="text-center">{{ __('Seri') }}</th>
                <th class="text-center">{{ __('Lint') }}</th> --}}
        <th width="10%" class="text-center">{{ __('Final') }}</th>
        <th width="5%" class="text-center">{{ __('Ket') }}</th>
      </tr>
      {{-- </thead> --}}
      {{-- <tbody> --}}
      @php
      ...
      @foreach ($participants as $index => $participant)
        @php
          ...
          if (!empty($participants[$index - 1])) {
              $pointBefore = $participants[$index - 1]->point_decimal ?? $participants[$index - 1]->point;
          }
          $currentPoint = $participant->point_decimal ?? $participant->point;
          if (!empty($pointBefore) && $pointBefore !== $currentPoint) {
              $rank++;
          }
        @endphp
        <tr {{-- @if ($loop->iteration == 1) style="font-size:130%" @elseif($loop->iteration == 2) style="font-size:120%" @elseif($loop->iteration == 3) style="font-size:110%" @endif> --}}>
          ...
          {{-- <td class="text-center">{!! $eventStage->number_format !!}</td>
