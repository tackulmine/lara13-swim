      ...
      <div class="table-responsive">
        ...
        <form action="{{ route($baseRouteName . 'destroy-batch') }}" method="post" class="mb-4" id="school-form">
          ...
          <button type="submit" class="btn btn-danger mb-4" data-toggle="tooltip" title="Delete selected rows">
            <i class="fa fa-trash-alt"></i> Batch Delete</button>
            ...
            <tbody>
              @foreach ($types as $type)
                <tr>
                  ...
                  <td>{{ $type->name }}</td>
                  <td class="text-center">{{ $type->event_stages_count }}</td>
                  <td class="text-center">{{ $type->event_registrations_count }}</td>
                  <td class="text-center">{{ $type->master_participants_count }}</td>
                  <td>
                    <a href="{!! route($baseRouteName . 'edit', $type->id) !!}" title="Edit {{ $type->name }}"
